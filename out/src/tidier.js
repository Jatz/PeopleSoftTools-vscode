...
es = require("event-stream"), path = require("path");
const vscode = require("vscode");
// var tmp = require("tmp");
var settings = vscode.workspace.getConfiguration("peoplesoft-tools-tidy");
module.exports = {
    errorLines(filePath) {
        return new Promise((resolve, reject) => {
            ...
            let tidiedFilePath = `${filePathDir}\\${fileNameWithoutExtension}_tidied${fileExt}`;
            ...
            var s = fs
                ...
                if (settings.removeHeaders) {
                    let results_psappsrv_header_type_1 = /(^PSAPPSRV.*?\d\.\d{6}\s)(.*)/.exec(line);
                    if (results_psappsrv_header_type_1) {
                        line = results_psappsrv_header_type_1[2];
                    }
                    let results_psappsrv_header_type_2 = /(^PSAPPSRV.*@JavaClient.*IntegrationSvc\]\(\d\)\s{3})(.*)/.exec(line);
                    ...
                    if (!traceType) {
                        let header_match_result = line.match(/PeopleTools\(\d+.\d+.\d+\) AE SQL\/PeopleCode Trace - \d\d\d\d-\d\d-\d\d/i);
                        if (header_match_result != null &&
                    ...
                    if (traceType == "AESQLPeopleCode") {
                        line = line.replace(/Line     Time       Elapsed Trace Data.*/, "");
                        line = line.replace(/-------- --------   ------- ------------->/, "");
                        line = line.replace(/^\d+\s+\d+:\d+:\d+\.\d+\s+\d+\.\d+\s/, "");
...
//# sourceMappingURL=tidier.js.map